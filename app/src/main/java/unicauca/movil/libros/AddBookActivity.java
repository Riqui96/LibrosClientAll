...
public class AddBookActivity extends AppCompatActivity implements Callback<SimpleResponse> {
    ...
    public void onResponse(Call<SimpleResponse> call, Response<SimpleResponse> response) {
        if(response.isSuccessful()){
            ...
            if(res.isSuccess()){
                Toast.makeText(AddBookActivity.this, "Libro Insertado !", Toast.LENGTH_SHORT).show();
                finish();
            }
            else{
                Toast.makeText(AddBookActivity.this, "Error Al Insertar Libro !", Toast.LENGTH_SHORT).show();
            }
        }
    }
